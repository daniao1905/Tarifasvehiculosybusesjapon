    ...
    try:
        ...
        if directions:
            ...
            if tipo_vehiculo in ["Hiace", "Alphard"]:
                ...
                min_extras = max(0, total_min - 180)
                bloques_km = math.ceil(km_extras / 7.5)
                bloques_tiempo = math.ceil(min_extras / 60)
                ...
                st.info(f"💴 {horas_cobradas}h × ¥{tarifa_hora} + {total_km:.1f}km × ¥{tarifa_km}")
                st.success(f"🧾 Total estimado: ¥{total:,}")
